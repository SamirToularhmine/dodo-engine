...
EditorEntity Scene::AddEntity(const char *_name)
{
  ...
  m_Registry.emplace<EntityComponent>(entity, EntityComponent{entity, MakeRef<Entity>(_name)});
...
void Scene::Shutdown()
{
  m_Registry.view<MeshComponent>().each([&](MeshComponent &_meshComponent)
                                        { _meshComponent.m_Model.reset(); });
}
...
void Scene::SelectEntity(EntityComponent &_entityComponent)
{
  ...
  m_SelectedEntity = MakeRef<EntityComponent>(_entityComponent);
}
