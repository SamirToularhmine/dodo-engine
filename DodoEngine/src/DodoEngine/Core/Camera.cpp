...
Camera::Camera(glm::vec3 _position) : m_Position(_position), m_ProjectionMatrix(1.0f), m_ViewMatrix(1.0f) {}
...
void Camera::UpdateProjectionMatrix(const glm::vec2& extent) {
  m_ProjectionMatrix = glm::perspective(glm::radians(90.0f), extent.x / static_cast<float>(extent.y), NEAR_PLANE, FAR_PLANE);
