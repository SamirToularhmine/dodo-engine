...
VulkanRenderer::VulkanRenderer(VulkanContext &_vulkanContext)
    : m_VulkanContext(_vulkanContext), m_UniformMvp({})
{
}
        ...
        [&](const MeshComponent &_meshComponent, const TransformComponent &_transformComponent)
        {
          ...
          std::for_each(
              ...
              [&](const Ref<Mesh> &_mesh)
              {
                ...
                RendererPushConstants rendererPushConstants{.m_ModelId = modelIndex++,
                                                            .m_TextureId = texture->GetId()};
                vkCmdPushConstants(commandBuffer, m_DefaultGraphicPipeline->GetPipelineLayout(),
                                   VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(RendererPushConstants),
                ...
                {
                  vkCmdDraw(commandBuffer, elementCount, 1, 0, 0);
                }
              });
        });
  }
