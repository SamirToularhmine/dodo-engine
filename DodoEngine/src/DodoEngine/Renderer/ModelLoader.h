...
struct ModelLoader
{
  static Ref<Model> LoadModel(const std::string& _modelFilePath, const SupportedModelFileType &_modelFileType)
  {
    switch (_modelFileType)
    {
    case SupportedModelFileType::OBJ:
      return ObjLoader::LoadFromFile(_modelFilePath);
    case SupportedModelFileType::FBX:
      return FbxLoader::LoadFromFile(_modelFilePath);
