...
VulkanSwapChain::VulkanSwapChain(const VulkanSurface& _vulkanSurface, const VulkanPhysicalDevice& _vulkanPhysicalDevice, Ref<VulkanDevice> _vulkanDevice, GLFWwindow* _window)
	: m_VulkanDevice(_vulkanDevice)
{
    // Surface capabilities
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(_vulkanPhysicalDevice, _vulkanSurface, &m_Spec.m_VkSurfaceCapabilities);

    // Surface formats
    uint32_t surfaceFormatCount;
    vkGetPhysicalDeviceSurfaceFormatsKHR(_vulkanPhysicalDevice, _vulkanSurface, &surfaceFormatCount, nullptr);

    if (surfaceFormatCount > 0) {
        std::vector<VkSurfaceFormatKHR>& surfaceFormats = m_Spec.m_VkSurfaceFormats;
        surfaceFormats.resize((surfaceFormatCount));
        vkGetPhysicalDeviceSurfaceFormatsKHR(_vulkanPhysicalDevice, _vulkanSurface, &surfaceFormatCount, surfaceFormats.data());
    }

    // Presentation modes
    uint32_t presentModeCount;
    vkGetPhysicalDeviceSurfacePresentModesKHR(_vulkanPhysicalDevice, _vulkanSurface, &presentModeCount, nullptr);

    if (presentModeCount > 0) {
        std::vector<VkPresentModeKHR>& presentModes = m_Spec.m_VkPresentModes;
        presentModes.resize(presentModeCount);
        vkGetPhysicalDeviceSurfacePresentModesKHR(_vulkanPhysicalDevice, _vulkanSurface, &presentModeCount, presentModes.data());
    }
    ...
    VkResult swapChainCreateResult = vkCreateSwapchainKHR(vulkanDevice, &swapChainCreateInfo, nullptr, &m_VkSwapChain);
    ...
    std::ranges::for_each(std::begin(m_Images), std::end(m_Images), [&](const VkImage& _image) {
        ...
        const VkResult createImageViewResult = vkCreateImageView(vulkanDevice, &imageViewCreateInfo, nullptr, &imageView);
