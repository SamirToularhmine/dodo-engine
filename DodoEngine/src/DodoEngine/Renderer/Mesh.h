...
struct Mesh {
  ...
  static Ref<Mesh> Create(const std::vector<Vertex>& _vertices, const std::vector<uint32_t>& _indices,
    ...
    Ref<VulkanBuffer> indicesBuffer =
        std::make_unique<VulkanBuffer>(VulkanBufferSpec{indicesAllocationSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VulkanBuffer::DEFAULT_MEMORY_PROPERTY_FLAGS});
  ...
  static Ref<Mesh> Create(const std::vector<Vertex>& _vertices, const char* _textureFileName = Texture::DEFAULT_TEXTURE_PATH) {
    // Vertices
    const uint32_t verticesAllocationSize = sizeof(Vertex) * _vertices.size();
    Ref<VulkanBuffer> vertexBuffer = std::make_unique<VulkanBuffer>(
        VulkanBufferSpec{verticesAllocationSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VulkanBuffer::DEFAULT_MEMORY_PROPERTY_FLAGS});
    vertexBuffer->SetMemory<Vertex>(_vertices.data(), verticesAllocationSize);

    return std::move(std::make_shared<Mesh>(std::move(vertexBuffer), nullptr, TextureManager::GetTexture(_textureFileName)));
  }
};
